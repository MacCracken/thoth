...
> **Status: 0.32.4 — the full spine wired, agentic tool-calling live, thoth reads *and writes* code (crash-safe),
...
> green/amber/red grounding indicator), degrading to a local `.thoth/memory/` flat file otherwise; a git producer (`/git`), and `web_fetch`/`web_search` via daimon+bote. As a **shell citizen**: a one-shot
