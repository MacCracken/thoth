...
row, `~/.thoth_history` after `/quit`, a talking and a silent `session_start`, `session_end` after `/quit` and after the
close), the 0.52.0 waits (a `sleep 20` hook mapped as running, Esc 0.06 s; `/run` followed a resize, Esc 0.11 s; the
close ended a `/run` in 0.12 s and a stream in 0.35 s with the partial kept and resumed), the pointer (thoth's own
arrow, clicks on conversation and tree rows), the thinking fold across a restart, `rainbow`. Running them found eight
defects, fixed with the pinned `hi` repaint: **a resize that raced a frame froze the window for good** (the present gate
waited on a release addressed to the destroyed buffer — it follows the attached buffer's id now, cleared by its
release or `delete_id`; 3 of 3 resize bursts froze 0.52.0, 0 of 3 this build); a scrolled feed painting over the
composer and strip (the raster clip is a stack); the authorization modal's `can)?run` question (one line, action first)
and its mid-word wrap; command cards drawn in every conversation (scoped by id, an ordinal anchor, clamped on a shrink);
the operator's line drawn twice while streaming; the composer keeping a running command's line; touchpad scrolling at
4× (the axis source read at the frame — Hyprland sends it after the axis; 1:1 for a finger, the notch keeps three
lines). HiDPI corrected: at scale 2 the window is upscaled soft, not small. Suite **755 + 1959 + 1032 + 823 + 190 + 5**
(+82); Linux / aarch64 / AGNOS warning sets identical to 0.52.0's; macOS green natively (822 + 1937 + 1029 + 190 + 749);
AGNOS runs `thoth 0.52.1` in ring 3 (the third boot — gnoboot's `ExitBootServices` flaked twice; repair batch 7).
...
the residual ADR-0022 and the config example named). `[deps].stdlib` gains `sys` (sigil 3.12.18 calls
`sys_uname`); the aarch64 lane loses two stale raw-syscall warnings. Every lane, macOS and the AGNOS runtime
re-verified at the new pin. Suite **647 + 1959 + 985 + 823 + 190 + 5** (+15); macOS green natively; AGNOS runs in
ring 3.
...
opens `UI_STATUS_INDENT` (2) columns in with U+F07B, the Nerd Font folder — 0.45.3's `📁` had no font on the
development machine and rendered as a two-column box, which is why the bar read as too far right. The GUI strip
matches (a flat folder drawn in one cell). The roadmap was re-cut: repairs batch into patch releases, minors are
held for feature arcs. Suite **324 + 1397 + 846 + 650 + 183 + 5** (+10); Linux / aarch64 / AGNOS build with 0.45.2's
warning set.

**0.45.3** — **the greeting is a block: the ibis beside a framed status box** (2026-09-13). thoth opens on its
mark — the Thoth ibis under the sun disk from `thoth_v1.tiff`, as coloured ASCII density art at 32 / 40 / 48 / 56
columns, GENERATED by `scripts/gen-splash.sh` (ImageMagick + awk; the ring dropped as the ink component with the
largest bounding box) into `src/splash.cyr` — beside a framed box: `{(o> Thoth - The Librarian: <version>`, an
open row, then the hoosh status + config notes (`src/greet.cyr`, the greeting's view-model + layout: owned spans,
...
starts a tab in with a folder before the location (0.45.4 moves it to two columns in behind a flat folder, and puts
the tagline under the title). One model, two painters: `tui_greeting_emit` (one feed line per
...
call `syscall(SYS_IOCTL, ...)`, and darshana's copy won the last-def, so four call sites issued syscall 16 on every
aarch64 build. The compiler had been saying so out loud (`duplicate symbol 'SYS_IOCTL' redefined with conflicting
value`) since 0.8.0 and nobody read aarch64 output. **darshana 1.0.0 removes its copy**; verified by building
`--aarch64` before and after, and pinned in `tests/cases/vendor.cyr`. The same removal dropped a `--win` error
(14 → 13). Second finding: **sit 1.6.1 broke the aarch64 build outright** (`undefined variable 'SYS_RENAME'` —
arm64 has no `rename` syscall, only `renameat`), fixed upstream as **sit 1.6.2** rather than patched thoth-side,
which also gave sit its own first working aarch64 build. Third: darshana 1.0.0 now rolls back its `SIG_BLOCK` when
`signalfd` fails, so `src/tui.cyr`'s unchecked `tty_open_signalfd` went from harmlessly-wrong to the exact terminal
...
documented collision seds.
`lib/` re-synced (30 replaced, 1 added, `cmp`-swept clean) with **no source migration required**. Static data
**10,990,792 → 999,864 bytes** (−91 %), warning set strictly shrank (11 gone, 0 new). Three new `sync-*.sh` scripts
close the hand-vendored gap; `tests/cases/vendor.cyr` adds 42 assertions gating the vendored floor. Suite 264 +
1591 + 183 + 3. macOS re-tested on real Apple Silicon and still fails — **pre-existing, not this refresh**: a
pristine `HEAD` baseline fails identically under the same compiler (see Targets).
...
- **Cyrius pin**: `6.6.4` (in `cyrius.cyml [package].cyrius`), moved from `6.6.3` at 0.51.3 (repair batch 5). The
  ...
  refused, not followed); sigil 3.12.18 gained an `agnosys_uname` → `sys_uname` call, so `[deps].stdlib` declares
  `sys` (dead-path in thoth, an undefined-fn warning otherwise); and the aarch64 lane lost two stale `raw syscall
  5` warnings (a `lib/dynlib.cyr` / `lib/fdlopen.cyr` fix upstream — a strict improvement). **Zero** removed
  symbols reachable, the suite green on the new pin, all three targets + macOS + the AGNOS runtime re-verified.
  Earlier: the `6.6.2` → `6.6.3` hop (0.45.6) refolds all twelve stdlib bundles (14 `lib/` files, 3013/1069
  lines); of the five thoth
  ...
  **8.84 MB**, which is **110 % of the old 8 MB slot** and **37 % of the new 24 MB one** (at 0.51.0 the
  ...
  6.5.20 (0.38.5) → **6.5.35** (0.38.6). Only ONE of those hops has ever forced a
  source migration: 6.4.78 → 6.5.20, where bayan 1.3.0 removed
  `bayan_json_v_parse_str` in favour of `_parse_buf` (34 sites, 9 files). The
  6.5.20 → 6.5.35 hop forced nothing, and that was **checked rather than assumed** —
  of the 21 public stdlib symbols removed between the two snapshots (the `err_*`
  constructor family, `str_starts_with_cstr`, some private helpers) thoth and its
  vendored bundles use none, and no symbol became `private` (6.5.0's file-scoped
  visibility feature is still unused across the entire snapshot). It carried a large
  win: **static data 10,990,792 → 999,864 bytes**, retiring the "13 MB
  vendored-sigil, unfixable from thoth" figure 0.37.0 recorded. Compiler tables at
  0.38.6, measured after the last edit: `fn_table 8292 / 32768`, `identifiers
  261884 / 524288`, `var_table 4624 / 8192`, `fixup_table 35364 / 1048576`.
  ⚠ **The shadow warning is not a completeness check** — `cyrius lib sync` has
  historically skipped equal-size files silently (8 stale modules at 0.38.2), so every
  refresh `cmp`-sweeps `lib/` file-by-file against the snapshot. 0.38.6: 30 replaced,
  1 added (`lib/async_macos.cyr`), sweep clean. Earlier history:
...
- **Presentation ladder** — `ui` + `surface` (the shared, tier-agnostic status **view-model**) + `greet` (the
