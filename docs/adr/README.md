...
- [0009 — Presentation capability ladder + tier-agnostic render surface](0009-presentation-capability-ladder.md) — *Accepted.* The TUI degrades through presentation tiers (T0 plain … T2 rich alt-screen) picked once at startup; the render surface is a semantic color-role API so features work at every tier and degrade closed when piped/CI.
