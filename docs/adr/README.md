...
- [0007 — The M5 avatara seam: native via a vendored dist bundle, persona threaded into the hoosh system prompt](0007-m5-avatara-seam-native-persona-system-prompt.md) — *Accepted.* M5 binds avatara native (a vendored dist bundle); the Thoth/Librarian persona is sourced from the archetype and threaded into the hoosh `{role:system}` message so it steers the turn (runtime-switchable since 0.22.x via `/persona`/`/role`).
- [0008 — Multi-target builds: one source tree, target picked at build time (Linux first)](0008-multi-target-builds.md) — *Accepted.* M6 wires `scripts/build.sh` to fan one source tree to targets; x86_64 Linux ships and AGNOS **builds** (the build-half blocker cleared upstream), with AGNOS-green pending a runtime host — degrades honestly, never faked.
...
- [0017 — Model `edit` tool: surgical, jailed, t-ron-gated, opt-in](0017-model-edit-tool-jailed-gated-opt-in.md) — *Accepted.* thoth can now WRITE code: opt-in (default off) `edit` (surgical unique-match replace) + `create_file` (create-only) tools, jailed to the launch cwd and gated under a distinct `thoth_edit` verb — the symmetric completion of the ADR-0015 read tools; a local capability, not a spine domain.
- [0018 — Subagent delegation: a scoped child *context*, not an orchestrated agent](0018-subagent-delegation-scoped-child-context.md) — *Accepted.* The model may `delegate(task)` to a child with the same tools and policy but its OWN empty context, depth-capped at 1, opt-in. Draws the spine line at **identity**: needs an identity, persona, registry entry or peer ⇒ orchestration (daimon/agnosai); one conversation's context, windowed ⇒ thoth's. Written as a FENCE — crews, presets, DAGs, inter-agent messaging and depth > 1 are explicit non-goals.
- [0019 — Layered config: `~/.thoth/` is the base, the project layers on top](0019-layered-config-global-base-local-override.md) — *Accepted.* Supersedes ADR-0016's discovery half: `~/.thoth/config.cyml` is a global base and the nearest `.thoth/config.cyml` overrides it PER KEY (the walk looks for the FILE, not the directory, so a `.thoth/` holding only `checkpoints/` no longer hides a repo's config). Memory layers the same way. Security rule: deny-lists UNION, `[shell].allow` and `[project].read_roots` are REPLACED — authority never accumulates from the less-trusted side.
